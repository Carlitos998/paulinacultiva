  ...
  try {
    ...
    if (!user) {
      return res.status(200).json({
        ...
        debugInfo: 'Usuario no encontrado, pero no lo revelamos por seguridad'
      });
    }
 ...
 */
const cleanExpiredTokens = async (req, res) => {
  try {
    const deleted = await PasswordResetToken.cleanExpiredTokens();
